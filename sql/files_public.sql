...
CREATE VIEW "volumes" AS SELECT files.vol_id, files.file_name AS volume_name, files.file_mod_date FROM files WHERE ((files.folder_id = 1) AND (files.file_mod_date = (SELECT max(b.file_mod_date) AS max FROM files b WHERE ((b.folder_id = 1) AND ((b.vol_id)::text = (files.vol_id)::text)))));

-- ----------------------------
--  View structure for "volumes_view"
-- ----------------------------
DROP VIEW IF EXISTS "volumes_view";
CREATE VIEW "volumes_view" AS SELECT files.vol_id, files.folder_id, files.file_name, volume_uuids.vol_uuid, volume_uuids.vol_total_capacity, volume_uuids.vol_available_capacity FROM (files JOIN volume_uuids ON (((files.vol_id)::text = (volume_uuids.vol_id)::text))) WHERE ((files.folder_id = 1) AND (files.file_mod_date = (SELECT max(b.file_mod_date) AS max FROM files b WHERE (((b.vol_id)::text = (volume_uuids.vol_id)::text) AND (b.folder_id = 1)))));
...
-- ----------------------------
--  View structure for "volume_names"
-- ----------------------------
DROP VIEW IF EXISTS "volume_names";
CREATE VIEW "volume_names" AS SELECT DISTINCT files.vol_id, files.file_name AS volume_name FROM files WHERE (files.file_id = 2);
